logme(1,"CyberScript: Gang module loaded")
...
function checkAlliesByScore(score)
	...
	if(score >= 5) then
		--logme(2,"Is potential ally")
		return true
		
		elseif(score < 0) then
		
		--logme(2,"Is not potential ally")
		return false
...
function setAttituteByScore(targetAttAgent,score, objlook)
	...
	--logme(2,"isFollower "..tostring(isFollower))
...
function GangAffinityCalculator()
	...
	local f = assert(io.open("mod/data/gameaffinity.json"))
	...
	for i=1,#getAffinity do 
		...
		logme(10,"testing fact "..affinity.fact.." result "..isquestok)
		if(isquestok == 1)then
			...
			if #affinity.npcscore > 0 then
				...
				--	logme(10,"added score "..tostring(score).." to NPC "..npcScore.Names)
