...
function BaseMappinBaseController_UpdateRootState(this)
        ...
        -- B-24 fix: this:GetMappin() can return nil when the mappin is being
        -- torn down, causing "attempt to index a nil value" on :GetWorldPosition().
        -- Wrap in pcall so a stale mappin controller can never crash this observer.
        local wordpos = nil
        local ok = pcall(function()
                wordpos = this:GetMappin():GetWorldPosition()
        end)
        if not ok or wordpos == nil then return end
