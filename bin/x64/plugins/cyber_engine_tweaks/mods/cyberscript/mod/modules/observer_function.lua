...
function WorldMapMenuGameController_OnUpdateHoveredDistricts(thos,district,subdistrict)
	...
	function MessengerGameController_PopulateData(thos) 
		...
		if(thos.f_sortOrder ~= nil or thos.sortOrder ~= nil) then
			...
			-- logme(10,JSON:encode_pretty(phoneConversation))
			-- if(getScoreKey(phoneConversation.tag,"unlocked") == nil or getScoreKey(phoneConversation.tag,"unlocked") == 1) then
			-- if phoneConversation.hash == nil then
	...
	function MessengerGameController_OnContactActivated(self,evt) 
		...
		logme(10,"MessengerGameController.OnContactActivated")
	...
	function PhoneMessagePopupGameController_SetupData(this)
		if(observerthread == true or moddisabled  == true)   then return end
		logme(1,"PhoneMessagePopupGameController.SetupData")
		if(currentPhoneConversation ~= nil) then
			this.dialogViewController = MessengerGameController
			
			  this.blackboard:SetBool(this.blackboardDef.isDisplayingMessage, false, true);
			
			if(this.m_data.mode == JournalNotificationMode.Menu) then
				this:UnregisterFromGlobalInputCallback("OnPostOnRelease", this, "OnHandleMenuInput");
			else
			  if this.data.openedFromPhone == false then
				PopupStateUtils.SetBackgroundBlur(this, false);
				this.uiSystem:PopGameContext(UIGameContext.ModalPopup);
				this.uiSystem:RestorePreviousVisualState("inkModalPopupState");
				this:SetTimeDilatation(false);
			 end
			  end
			-- local test = gameJournalPhoneMessage.new()
			-- thos.dialogViewController:ShowThread(test)
	...
	function MessengerDialogViewController_ShowThread(self,thread)
		...
		if(thread == nil) then
			local message = {}
			logme(10,currentPhoneConversation)
	...
	function MessengerDialogViewController_UpdateData(self,animateLastMessage,setVisited)
		...
		logme(10,"MessengerDialogViewController_UpdateData")
		if(currentPhoneConversation ~= nil) then
		if(contact == nil) then
			local message = {}
			logme(10,"test3")
	...
	function MessengerDialogViewController_ActivateReply(self,target)
		...
		if(currentPhoneConversation ~= nil) then
			for i = 1, #currentPhoneConversation.currentchoices do
				logme(10,currentPhoneConversation.currentchoices[i].text)
				logme(10,target.labelPathRef:GetText())
