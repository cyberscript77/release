...
function windowsManager() -- manage and toggle UI windows
	...
	if draw and  initFinished then--others windows
		...
		if(overlayOpen ) then
			...
			if file_exists("data/modules/editor.lua") then
				newWindows()
			end
	...
	if ImGui.BeginTabItem("Command") then
		local status, result =  pcall(function()
			...
			if CPS:CPButton("Finish game quest") then
				...
				logme(6,"Current game active quest ended")
			end
			ImGui.Spacing()
			...
			if CPS:CPButton("Get mission by trigger") then
				getMissionByTrigger()
				logme(6,"enabled mission"..#possibleQuest)
			end
			ImGui.Spacing()
			ImGui.Spacing()
			ImGui.Spacing()
			...
			if CPS:CPButton("check quest conditions") then
				...
				logme(6,testTriggerRequirement(qu.trigger_condition_requirement,qu.trigger_condition))
				logme(6,testTriggerRequirement(qu.start_condition_requirement,qu.start_condition))
				logme(6,testTriggerRequirement(qu.end_condition_requirement,qu.end_condition))
				logme(6,testTriggerRequirement(qu.failure_condition_requirement,qu.failure_condition))
			end
			ImGui.Spacing()
...
function HousingWindows()
	-- logme(6,tostring(currentHouse ~= nil))
	-- logme(6,tostring(isInHouse))
