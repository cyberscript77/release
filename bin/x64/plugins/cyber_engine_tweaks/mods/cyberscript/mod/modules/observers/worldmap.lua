...
function WorldMapTooltipController_SetData(self,data,menu)
        ...
        -- B-15 fix: gate the gang-info / fixer world-map override behind an
        -- opt-in user setting `world_map_gang_info_override`. v5.1.4 removed
        -- this override; the code path was still present in the fork and ran
        -- unconditionally. We preserve the fork's current behavior by defaulting
        -- the setting to true, but downstream mods / users who want the
        -- "removed" v5.1.4 behavior can set it to false in
        -- user/settings/cyberscript.json.
        local gangInfoOverride = true
        if getUserSettingWithDefault then
                gangInfoOverride = getUserSettingWithDefault("world_map_gang_info_override", true)
        end
        if gangInfoOverride ~= true then
                -- Override disabled by user / downstream mod — skip the rest
                -- of this function so the world map shows vanilla tooltips.
                return
        end
