...
local function writeSessionFile(sessionTimestamp, sessionKey, isTemporary, sessionData)
	...
	local sessionFile = io.open(sessionDataDir .. '/latest.txt', 'w')
	if not sessionFile then
		raiseError(('Cannot write session file %q.'):format(sessionPath))
	end
	
	sessionFile:write(sessionPath)
...
function GameSession.readLatest()
	
		local f = io.open(sessionDataDir.."/latest.txt")
		local lines = f:read("*a")
		
		f:close()
		
		if(lines ~= "" and file_exists(lines)) then
			sessionPath = lines
			local sessionChunk = loadfile(sessionPath)
			...
			local sessionData = sessionChunk()
